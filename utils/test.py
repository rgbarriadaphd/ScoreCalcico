...
def evaluate_model(model, dataloader, device, log, epoch=None, fold=None):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            acierto = 1 if predicted == ground else 0
            sample_name = file_info[0][0]
            class_name = sample_name.split('/')[3]
            sample_name = sample_name = os.path.splitext(sample_name)[0].split('/')[-1]
            prediction.append((sample_name, predicted.item(), np.array(outputs.cpu())[0][0], np.array(outputs.cpu())[0][1], acierto, np.array(ground.cpu())[0], fold, class_name))
            total += ground.size(0)
